...
const createBuffer = (data, indexes = false)=> {
    if(contextReady) {
        let buffer =  gl.createBuffer();
        gl.bindBuffer(indexes? gl.ELEMENT_ARRAY_BUFFER : gl.ARRAY_BUFFER, buffer);
        gl.bufferData(indexes? gl.ELEMENT_ARRAY_BUFFER : gl.ARRAY_BUFFER, indexes ? new Uint16Array(data) : new Float32Array(data),  gl.STATIC_DRAW);
        gl.bindBuffer(indexes? gl.ELEMENT_ARRAY_BUFFER : gl.ARRAY_BUFFER, null);
        return buffer;
    } else {
        console.log(new Error("Context not set yet"));
    }
